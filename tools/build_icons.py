...
was measured from the reference artwork (a 2000px PNG) and is expressed on a
108-unit canvas, so the same numbers drive the SVG favicon and every raster
size.
...
BG_FLAT = "#0B0F0F"  # flat background, for the maskable icon
...
def rounded_rect_path(x, y, w, h, r):
    """SVG path data for a rectangle with rounded corners."""
    r = min(r, w / 2, h / 2)
...
def main():
    ...
    print(f"maskable scale {safe_scale(CANVAS * 0.8)}")
